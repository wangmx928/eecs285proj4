...
public class ClientSender
{
      ...
      else
      {
    	  ...
    	  while(i < newRequestList.length() - 1)
    	  {
    	    i++;
    		  ...
    		  Integer invoteCount;
    		  read = "";
    		  for(i = i + 1; newRequestList.charAt(i) != '-'; i++)
    			  read = read + newRequestList.charAt(i);
    		  insongTitle = read;

    		  read = "";
    		  for(i = i + 2; newRequestList.charAt(i) != '#'; i++)
    			  read = read + newRequestList.charAt(i);
    		  inartist = read;
    		  ...
    		  for(++i; newRequestList.charAt(i) != '\n'; i++)
    			  read = read + newRequestList.charAt(i);
    		  invoteCount = Integer.parseInt(read);
    ...
    public void updatePlaylist(String newPlaylist)
    {
    	...
    	{
    		//ERROR
    		System.out.println("Error in updatePlaylist Function: not receiving 'PLAYLIST: '"
    				+ "\nshowing word: " + read);
    	}
    	else
    	{
    		while(i < newPlaylist.length() - 1)
    		{
    		  i++;
    		  read = "";
    		  /*for (i = i + 1; newPlaylist.charAt(i) != '-'; i++)
    		  {
    		    read = read + newPlaylist.charAt(i);
    		  }
    		  read = read + " - ";
    		  for (i = i + 4; newPlaylist.charAt(i) != '\n'; i++)
    		  {
    		    read = read + newPlaylist.charAt(i);
    		  }*/
